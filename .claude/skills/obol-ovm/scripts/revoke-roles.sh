#!/usr/bin/env bash
# Revoke roles on an OVM contract
# Usage: ./revoke-roles.sh <ovm_address> <target_address> <roles_value> [network]
#
# Roles (bitwise flags):
...
OVM="${1:?Usage: revoke-roles.sh <ovm_address> <target_address> <roles_value> [network]}"
TARGET="${2:?Missing target address}"
ROLES="${3:?Missing roles value}"
NETWORK="${4:-mainnet}"
...
echo "Revoking roles on $NETWORK..."
echo "  OVM:    $OVM"
echo "  Target: $TARGET"
echo "  Roles:  $ROLES"
echo ""
