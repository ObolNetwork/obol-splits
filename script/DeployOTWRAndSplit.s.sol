...
contract DeployOTWRAndSplit is Script, SplitterConfiguration {
    ...
    function run(
        ...
        for (uint256 i = 0; i  < data.length; i++) {
            ...
            // create obol split
            address otwrAddress = address(OptimisticTokenWithdrawalRecipientFactory(OTWRFactory).createOWRecipient(
                token,
                address(0),
    ...
    function _validateInputJson(ConfigurationData[] memory configuration) internal pure {
        for (uint256 i = 0; i < configuration.length; i++) {
            if (configuration[i].principalRecipient == address(0)) {
                revert Invalid_PrincipalRecipient();
            }
