/**
 * OVM Revoke Roles Tool - Revoke RBAC roles on an OVM
 */
...
export const tool: Tool = {
  ...
  description:
    "Revoke roles from an address on an Obol Validator Manager (OVM). Available roles: WITHDRAWAL_ROLE, CONSOLIDATION_ROLE, SET_BENEFICIARY_ROLE, RECOVER_FUNDS_ROLE, SET_REWARD_ROLE, DEPOSIT_ROLE. Returns Cast command and encoded calldata.",
  inputSchema: {
    type: "object" as const,
    properties: {
      ovmAddress: { type: "string", description: "OVM contract address" },
      targetAddress: { type: "string", description: "Address to revoke roles from" },
      roles: {
        type: "array",
        items: { type: "string" },
        description: "Roles to revoke (e.g. ['WITHDRAWAL_ROLE', 'DEPOSIT_ROLE'])",
      },
  ...
  --private-key $PRIVATE_KEY`;
    ...
    return JSON.stringify({
      ...
      transactionData: {
        ...
        data: encodedData,
        value: "0",
        description: `Revoke roles ${roles.join(", ")} from ${targetAddress}`,
      },
      castCommand,
    ...
    return JSON.stringify({ error: `Failed to prepare revoke roles: ${message}` });
  }
}
