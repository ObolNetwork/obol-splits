...
contract ObolCollectorTest is Test {
    ...
    function test_feeShare() public {
        assertEq(
            collectorWithFee.feeShare(),
            feeShare,
            "invalid collector fee"
        );

        assertEq(
            ethCollectorWithFee.feeShare(),
            feeShare,
            "invalid collector value fee"
        );
    }

    function test_feeRecipient() public {
        assertEq(
            collectorWithFee.feeRecipient(),
            feeRecipient,
            "invalid collector feeRecipient"
        );
        
        assertEq(
            ethCollectorWithFee.feeRecipient(),
            feeRecipient,
            "invalid collector feeRecipient 2"
        );
    }

    function test_withdrawalAddress() public {
        assertEq(
            collectorWithFee.withdrawalAddress(),
            withdrawalAddress,
            "invalid split wallet"
        );

        assertEq(
            ethCollectorWithFee.withdrawalAddress(),
            ethWithdrawalAddress,
            "invalid eth split wallet"
        );
    ...
    function test_token() public {
        assertEq(
            collectorWithFee.token(),
            address(mERC20),
            "invalid token"
        );

        assertEq(
            ethCollectorWithFee.token(),
            address(0),
            "ivnalid token eth"
        );
    }
    ...
    function test_DistributeETHWithFee() public {
        ...
        assertEq(
            address(ethWithdrawalAddress).balance,
            amountToDistribute - fee,
            "invalid amount to split"
        );
    }
    ...
    function test_RescueTokens() public {
        ...
        assertEq(
            rescueERC20.balanceOf(withdrawalAddress),
            amountToRescue,
            "invalid erc20 amount"
        );
        ...
        assertEq(
            rescueERC20.balanceOf(ethWithdrawalAddress),
            amountToRescue,
            "invalid erc20 amount"
        );
    }
