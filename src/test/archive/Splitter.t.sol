...
contract SplitterTest is DSTest {
  ...
  function testCallDeploy() public {
    ...
    assertEq(mockSplitter.showSplitAllocation(0), 320_000);
    assertEq(mockSplitter.showSplitAllocation(1), 320_000);
    assertEq(mockSplitter.showSplitAllocation(2), 320_000);
    assertEq(mockSplitter.showSplitAllocation(3), 40_000);
  ...
  function testCallDeploy7() public {
    ...
    for (uint256 i = 0; i < splitCnt; i++) {
      assertEq(mockSplitter.showSplitAllocation(i), 137_142);
    }
    assertEq(mockSplitter.showSplitAllocation(splitCnt), 40_006);
