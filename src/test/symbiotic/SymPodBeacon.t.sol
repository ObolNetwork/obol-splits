...
contract SymPodBeaconTest is Test {
  ...
  function setUp() public {
    ...
    symPodBeacon = new SymPodBeacon(symPodImplementation, owner);
  }

  function test_CannotCreateWithInvalidOwner() external {
    vm.expectRevert();
    new SymPodBeacon(symPodImplementation, address(0));
  }

  function test_CannotCreateWithInvalidImpl() external {
    vm.expectRevert();
    new SymPodBeacon(address(0), owner);
  }
  ...
  function test_UpgradeTo() external {
    ...
    assertEq(symPodBeacon.implementation(), newSymPodImpl, "could not upgrade implementation");
  }

  function test_Owner() external {
    assertEq(symPodBeacon.owner(), owner, "invalid owner address");
  }
  ...
  function test_transferOwnership() external {
    ...
    assertEq(symPodBeacon.owner(), newOwner, "Invalid new owner");
  }
  ...
  function test_renounceOwnership() external {
    ...
    assertEq(symPodBeacon.owner(), address(0), "could not renounce ownership");
  }

  function test_implementation() external {
    assertEq(symPodBeacon.implementation(), symPodImplementation, "invalid implementation");
  }
}
