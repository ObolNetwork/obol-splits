...
contract BaseSymPodTest is Test {
    ...
    function getTotalValidatorBalances(uint40[] memory validatorIndices, bytes32[] memory balancesRoots) internal pure returns (uint256 totalBalance) {
...
contract SymPod__Initialize is BaseSymPodTest {
    ...
    function test_CanInitialize() external {
        ...
        assertEq(
            podImplementation.slasher(),
            slasher,
            "invalid slasher"
        );

        assertEq(
            podImplementation.admin(),
            podAdmin,
            "invalid slasher"
        );
        
        assertEq(
            podImplementation.withdrawalAddress(),
            withdrawalAddress,
            "invalid slasher"
        );

        assertEq(
            podImplementation.recoveryAddress(),
            recoveryRecipient,
            "invalid slasher"
        );
    }
...
contract SymPod__RecoverTokens is BaseSymPodHarnessTest {
    ...
    function test_RecoverTokens() external {
        ...
        assertEq(
            airdropToken.balanceOf(createdPod.recoveryAddress()),
            amountToMint,
            "invalid amount"
        );
    }
...
contract SymPod__StartCheckPoint is BaseSymPodHarnessTest {
    ...
    function test_StartCheckpointWithBalance() external {
        ...
        assertEq(
            currentCheckpoint.podBalanceGwei,
            amountOfEther / 1 gwei,
            "invalid pod balance"
        );
    ...
    function testFuzz_StartCheckPoint(
        uint8 numberOfValidators,
        uint128 amountOfEther
    ) external {
    ...
    function testFuzz_StartMultipleCheckPoint(
        ...
        assertEq(
            createdHarnessPod.withdrawableRestakedPodWei(),
            roundDown(amountToDeal * 2) ,
            "invalid withdrawable balance"
        );
    }
}
...
contract SymPod__InitWithdraw is BaseSymPodHarnessTest {
    ...
    function testFuzz_CanInitWithdraw(
        uint256 amount,
        uint256 nonce
    ) external {
        vm.assume(amount > 0);
...
contract SymPod__VerifyWithdrawalCredentials is BaseSymPodHarnessTest {
    ...
    string verifyWithdrawalCredentialProofPath = "./src/test/test-data/mainnet/VerifyWithdrawalCredential-proof_deneb_mainnet_slot_9575417.json";
    string verifyExitedWithdrawalCredentialProofPath = "./src/test/test-data/mainnet/VerifyWithdrawalCredential-exited_proof_deneb_mainnet_slot_9575417.json";
...
contract SymPod__VerifyBalanceCheckpoints is BaseSymPodHarnessTest {
    ...
    string verifyWithdrawalCredentialProofPath = "./src/test/test-data/mainnet/VerifyWithdrawalCredential-proof_deneb_mainnet_slot_9575417.json";
    string validatorBalanceProofPath = "./src/test/test-data/mainnet/VerifyBalanceCheckpointProof-proof_deneb_mainnet_slot_9575417.json";
    ...
    function testFuzz_CanVerifyBalanceCheckpoint(uint256 podBalance) external {
        ...
        uint256 firstValidatorBalanceGwei = BeaconChainProofs.getBalanceAtIndex(
            validatorBalancesProof.validatorBalanceRoots[0],
            validatorProof.validatorIndices[0]
        );

        uint256 newTotalValidatorBalanceMinusFirstValidatorGwei = getTotalValidatorBalances(
            validatorProof.validatorIndices,
            validatorBalancesProof.validatorBalanceRoots
        ) - firstValidatorBalanceGwei;

        int256 expectedBalanceDeltaGwei = int256(newTotalValidatorBalanceMinusFirstValidatorGwei) - int256(currentTotalValidatorBalanceGwei);
        ...
        assertEq(
            currentCheckpoint.proofsRemaining,
            1,
            "invalid number of proofs remaining"
        );
        assertEq(
            currentCheckpoint.podBalanceGwei,
            podBalanceGwei,
            "invalid pod balance gwei"
        );
        assertEq(
            expectedBalanceDeltaGwei,
            currentCheckpoint.balanceDeltasGwei,
            "invalid balance delta"
        );
        ...
        int256 totalValidatorsBalanceWei = (int256(newTotalValidatorBalanceMinusFirstValidatorGwei + firstValidatorBalanceGwei) * 1 gwei);
        int256 expectedTotalShareDeltaWei = totalValidatorsBalanceWei - int256((32 ether * sizeOfValidators));
        vm.expectEmit(true, true, true, true);
        emit ValidatorBalanceUpdated(
            sampleValidatorIndex,
            block.timestamp,
            32 gwei,
            firstValidatorBalanceGwei
        );
        ...
        vm.expectEmit(true, true, true, true);
        emit ValidatorCheckpointUpdate(
            block.timestamp,
            sampleValidatorIndex
        );
        ...
        emit IncreasedBalance(
            uint256(totalValidatorsBalanceWei + podBalanceWei),
            uint256(expectedTotalShareDeltaWei + podBalanceWei)
        );
        ...
        assertEq(
            ...
            "invalid withdrawable restaked pod wei"
        );
    }
}
...
contract SymPod__VerifyExpiredBalance is BaseSymPodHarnessTest {
    string expiredBalanceProofFilePath = "./src/test/test-data/mainnet/VerifyExpiredBalanceProof-proof_deneb_mainnet_slot_9575417.json";
    ...
    function test_CannotVerifyIfValidatorNotSlashed() external {
        string memory notSlashedExpiredBalanceProofPath =  "./src/test/test-data/mainnet/VerifyExpiredBalanceProofNotSlashed-proof_deneb_mainnet_slot_9575417.json";
        proofParser.setJSONPath(expiredBalanceProofFilePath);
        uint40 validatorIndex = uint40(proofParser.getValidatorIndex());
        ...
        bytes32 currentValidatorPubKeyHash = localValidatorFieldsProof.validatorFields[BeaconChainProofs.VALIDATOR_PUBKEY_INDEX];
...
contract SymPod__VerifyExceedBalanceDelta is BaseSymPodHarnessTest {
    ...
    string expiredBalanceProofFilePath = "./src/test/test-data/mainnet/VerifyExceedBalanceDelta-proof_deneb_mainnet_slot_9575417.json";
    ...
    function test_CannotVerifyIfInvaldBeaconTimestamp() external {
        createdHarnessPod.changeValidatorLastCheckpointedAt(
            balanceProof.validatorPubKeyHash,
            timestamp + 10
        );
    ...
    function test_CannotVerifyInvalidBalanceDelta() external {
        string memory proofPath =  "./src/test/test-data/mainnet/VerifyExceedBalanceDeltaNotExited-proof_deneb_mainnet_slot_9575417.json";
