...
contract OELPCIntegration is EigenLayerTestBase {
    ...
    function setUp() public {
        ...
        OptimisticWithdrawalRecipientFactory owrFactory = new OptimisticWithdrawalRecipientFactory(
            "demo.obol.eth",
            ENS_REVERSE_REGISTRAR_GOERLI,
            address(this)
        );

        owrWithdrawalAddress = address(owrFactory.createOWRecipient(
            address(0),
            principalRecipient,
            rewardRecipient,
            32 ether
        ...
        owrController = ObolEigenLayerPodController(factory.createPodController(
            owner,
        ...
        splitController = ObolEigenLayerPodController(factory.createPodController(
            owner,
            splitWithdrawalAddress
        ));
    ...
    function testFuzz_WithdrawOWR(uint256 amountToDeposit) external {
        ...
        vm.startPrank(owner);
        {
            owrController.callEigenPod(
                encodeEigenPodCall(address(owrController), amountToDeposit)
            );
        ...
        assertEq(
            ...
            fee,
            "fee recipient balance increased"
        );

        uint256 owrBalance = amountToDeposit - fee;
        assertEq(
            address(owrWithdrawalAddress).balance,
            owrBalance,
            "owr balance increased"
        );
    ...
    function testFuzz_WithdrawSplit(uint256 amountToDeposit) external {
        ...
        vm.startPrank(owner);
        {
            splitController.callEigenPod(
                encodeEigenPodCall(address(splitController), amountToDeposit)
            );
        ...
        assertEq(
            address(splitWithdrawalAddress).balance,
            splitBalance,
            "invalid balance"
        );
    }
