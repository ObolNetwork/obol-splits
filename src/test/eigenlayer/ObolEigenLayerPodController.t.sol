...
contract ObolEigenLayerPodControllerTest is Test {
    ...
    function test_CallEigenPod() external {
        ...
        vm.prank(owner);
        controller.callEigenPod(
            encodeEigenPodCall(user1, amount)
        );
    }

    function test_CallDelegationManager() external {
        vm.prank(owner);
        controller.callDelegationManager(
            encodeDelegationManagerCall(
                EIGEN_LAYER_OPERATOR_GOERLI
            )
        );
    }
    ...
    function test_OnlyOwnerCallDelegationManager() external {
        ...
        vm.expectRevert(Unauthorized.selector);
        controller.callDelegationManager(
            encodeDelegationManagerCall(EIGEN_LAYER_OPERATOR_GOERLI)
        );
    }
    ...
    function test_CallEigenPodManager() external {
        ...
        vm.prank(owner);
        controller.callEigenPodManager{value: etherStake}(
            encodeEigenPodManagerCall(0)
        );
    }

    function test_OnlyOwnerEigenPodManager() external {
        vm.expectRevert(Unauthorized.selector);
        controller.callEigenPodManager(
            encodeEigenPodManagerCall(0)
        );
    }
    ...
    function test_ClaimDelayedWithdrawals() external {
        ...
        {
            controller.callEigenPod(
                encodeEigenPodCall(address(controller), amountToDeposit)
            );
