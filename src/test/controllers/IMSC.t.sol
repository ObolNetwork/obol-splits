...
contract IMSC is Test {
  ...
  function setUp() public {
    ...
    split = ISplitMain(SPLIT_MAIN_HOLESKY).createSplit(accounts, percentAllocations, 0, predictedControllerAddress);
  ...
  function testCan_getSplitMain() public {
    assertEq(controller.splitMain(), SPLIT_MAIN_HOLESKY, "valid splitMain address");
  }
  ...
  function testCan_updateSplit() public {
    ...
    assertEq(
      ISplitMain(SPLIT_MAIN_HOLESKY).getHash(split),
  ...
  function testFuzz_updateSplit(
    ...
    // create split
    address fuzzSplit =
      ISplitMain(SPLIT_MAIN_HOLESKY).createSplit(splitterAccts, splitterPercentAlloc, 0, predictedControllerAddress);
    ...
    bytes32 currentSplitHash = ISplitMain(SPLIT_MAIN_HOLESKY).getHash(fuzzSplit);
    // update split
    vm.prank(ownerAddress);
    controller.updateSplit();

    bytes32 newSplitHash = ISplitMain(SPLIT_MAIN_HOLESKY).getHash(fuzzSplit);
