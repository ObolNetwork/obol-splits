 ...
 * via hashing when needed. Each split gets its own address & proxy for maximum composability with other contracts
 * onchain.
...
contract SplitMainV2 is ISplitMainV2 {
  ...
  modifier onlySplitFactory() {
    if (msg.sender != splitFactory) revert Unauthorized(msg.sender);
    _;
  }
  ...
  modifier checkIfRequiresDistributor(address split) {
    ...
    if (distributor != address(0) && distributor != msg.sender) revert Unauthorized(msg.sender);
    _;
  }
   ...
   *  @dev Two-step control transfer inspired by
   * [dharma](https://github.com/dharma-eng/dharma-smart-wallet/blob/master/contracts/helpers/TwoStepOwnable.sol)
  ...
  function withdraw(address account, uint256 withdrawETH, ERC20[] calldata tokens) external override {
    ...
    uint256 ethAmount;
    if (withdrawETH != 0) ethAmount = _withdraw(account);
    unchecked {
      // overflow should be impossible in for-loop index
   ...
   */
  function _scaleAmountByPercentage(uint256 amount, uint256 scaledPercent) internal pure returns (uint256 scaledAmount) {
